<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:param name="action">NOT_DEFINED</xsl:param>
    <xsl:param name="Username-Action">anonymous</xsl:param>
    <xsl:param name="X-Forwarded-For-Action">10.0.0.1</xsl:param>
    <xsl:template match="/">
        <xsl:apply-templates />
    </xsl:template>
    <xsl:template match="//jsonObject">
        <jsonObject>
            <xsl:if test="$action != 'NOT_DEFINED'">
                <action>
            ...
            <variables>
                <name>usernameAction</name>
                <value>
                    <xsl:value-of select="$Username-Action"/>
                </value>
            </variables>
            <variables>
                <name>xForwardedForAction</name>
                <value>
                    <xsl:value-of select="$X-Forwarded-For-Action"/>
                </value>
            </variables>
        </jsonObject>
    </xsl:template>
