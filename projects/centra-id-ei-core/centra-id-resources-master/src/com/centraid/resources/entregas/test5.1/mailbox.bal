    ...
    resource function newMailbox(http:Caller caller, http:Request req) {
        ...
        if( payload is json ) {
            ...
            if(_mailbox is Mailbox ) {
                ...
                string _uuid = enableMailbox(<@untainted> _mailbox.identity,payload );
    ...
    resource function deleteMailbox(http:Caller caller, http:Request req) {
        ...
        if( payload is json ) {
            ...
            if(_mailbox is Mailbox ) {
                ...
                string _uuid = disableMailbox(<@untainted> _mailbox.identity, payload );
...
function enableMailbox(string identity, json mailbox ) returns string {
    ...
    string path = _path + "/" + identity + ".json";
...
function disableMailbox(string identity, json mailbox ) returns string {
    // Get path to store the new request
